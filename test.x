<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">

    <meta property="og:title" content="WAKE — The JIT Engine for Live Execution">
    <meta property="og:description" content="Wake is a real-time JIT engine for building interfaces, tools, and creative systems. Fast. Native. Alive.">
    <meta property="og:image" content="https://wake.tools/assets/wake-preview.png">
    <meta property="og:url" content="https://wake.tools/">
    <meta property="og:type" content="website">

    <meta name="twitter:card" content="summary_large_image">
    <meta name="twitter:title" content="WAKE — The JIT Engine for Live Execution">
    <meta name="twitter:description" content="Wake is a real-time JIT engine for comprehensive interfaces, tools, and creative systems.">
    <meta name="twitter:image" content="https://wake.tools/assets/wake-preview.png">

    <title>WAKE JIT Engine - The Power of a Live Engine</title>
    <script src="https://cdn.tailwindcss.com"></script>
    <link rel="stylesheet" href="https://fonts.googleapis.com/css2?family=JetBrains+Mono:wght@400;700&family=Inter:wght@300;400;600&display=swap">
    <script>fetch("index.html?_="+Date.now(),{cache:"no-store"}).then(r=>r.text()).then(t=>//i.test(t)&&RegExp.$1.trim()!=="v0003" &&location.reload());</script>
                                                                                                                                                                    <style>
        body {
            font-family: 'Inter', sans-serif;
            background-color: #000000;
            color: #EAEAEA;
            overflow-x: hidden;
        }
        .mono {
            font-family: 'JetBrains Mono', monospace;
        }
        .wake-blue {
            color: #0066CC;
        }
        .bg-wake-blue {
            background-color: #0066CC;
        }
        .hover-bg-wake-blue:hover {
            background-color: #0055AA;
        }
        .console-text {
            opacity: 0;
            animation: fadeIn 0.3s forwards;
        }
        @keyframes fadeIn {
            to { opacity: 1; }
        }
        .slide-up {
            opacity: 0;
            transform: translateY(20px);
            transition: opacity 0.6s, transform 0.6s;
        }
        .slide-up.visible {
            opacity: 1;
            transform: translateY(0);
        }

        /* --- Global Gallery Styles (cleaned up) --- */
        .module-card {
            background-color: rgba(0, 0, 0, 0.2);
            border: 1px solid #0066CC;
            backdrop-filter: blur(5px);
            transition: all 0.3s ease;
        }
        .module-card:hover {
            transform: translateY(-5px);
            box-shadow: 0 0 20px rgba(0, 102, 204, 0.3);
            background-color: rgba(0, 0, 0, 0.4);
        }
        .manifesto-line {
            opacity: 0;
            transform: translateY(10px);
            transition: opacity 0.6s, transform 0.6s;
        }
        .manifesto-line.visible {
            opacity: 1;
            transform: translateY(0);
        }

        /* --- Console & Blur Layer Styles --- */
        #console-wrapper {
            position: absolute;
            top: 0;
            left: 0;
            width: 100%;
            height: 100%;
            z-index: 0;
            pointer-events: none;
        }

        #console-container {
            position: absolute;
            top: 0;
            left: 0;
            width: 100%;
            padding: 1rem;
            box-sizing: border-box;
            background-color: rgba(0, 0, 0, 0.3);
            will-change: transform;
            pointer-events: auto;
        }

        #console-blur-overlay {
            position: absolute;
            top: 0;
            left: 0;
            width: 100%;
            height: 100%;
            backdrop-filter: blur(0px); /* Initial blur, will be updated by JS */
            pointer-events: none;
            z-index: 1;
            background-color: rgba(0, 0, 0, 0.5);
            transition: backdrop-filter 0.1s ease-out;
        }

        #console-container #console-output {
            background-color: transparent;
            padding: 0;
            border-radius: 0;
            display: block;
            position: relative;
            z-index: 0;
        }

        #main-content {
            opacity: 0;
            transition: opacity 0.8s ease-in;
            position: relative;
            z-index: 1;
        }

        /* Style pour le contour du titre */
        .title-outline {
            text-shadow:
                -2px -2px 0 #000,
                2px -2px 0 #000,
                -2px  2px 0 #000,
                2px  2px 0 #000;
        }

        /* --- Frameless Slideshow Specific Styles --- */
        .frameless-slideshow-container {
          position: relative;
          width: 100%;
        height: 700px;
          overflow: hidden;
          cursor: grab;
        }

        .frameless-slideshow-container.grabbing {
            cursor: grabbing;
        }

      .frameless-slideshow-wrapper {
    position: relative;
    width: 100%;
 height: auto;

}
        
        /* Classe pour désactiver temporairement la transition */
        .frameless-slideshow-wrapper.no-transition {
            transition: none !important;
        }


        .frameless-slide {
            position: absolute;
          top: 0;
          left: 0;
          user-select: none;
          width: 100%; /* ou une largeur fixe si tu préfères */
          height: 100%;
         // display: flex;
          justify-content: center;
          align-items: center;
        
        }

.frameless-slide img {
    max-width: 100%; /* Permet à l'image de prendre sa largeur naturelle, mais ne pas dépasser 100% de son parent */
    height: auto;    /* Maintient le ratio d'aspect de l'image */
    object-fit: contain; /* Assure que l'image entière est visible à l'intérieur de son conteneur */
    user-select: none;
    display: block; /* Élimine les espaces blancs sous l'image */
    margin: 0 auto; /* Centre l'image horizontalement dans la diapositive */
}

        /* Overlay for darkening effect on side images */
        .frameless-slide::after {
            content: '';
            position: absolute;
            top: 0;
            left: 0;
            width: 100%;
           height: auto;
            object-fit: contain;
            
            background: rgba(0, 0, 0, 0); /* Start transparent */
            pointer-events: none; /* Allow clicks/interactions on image below */
            transition: background 0.6s ease-out; /* Smooth transition for overlay */
        }

        /* Active (center) slide styles */
        .frameless-slide.active img {
            filter: brightness(1); /* Full brightness */
            transform: scale(1); /* Full scale */
        }

        .frameless-slide.active::after {
            background: rgba(0, 0, 0, 0); /* No overlay */
        }

        /* Side previews - first level */
        .frameless-slide.side-1 img {
            filter: brightness(0.7); /* Slightly darker */
            transform: scale(0.9); /* Slightly smaller */
        }
        .frameless-slide.side-1::after {
            background: rgba(0, 0, 0, 0.3); /* Slight overlay */
        }

        /* Side previews - second level */
        .frameless-slide.side-2 img {
            filter: brightness(0.5); /* More darker */
            transform: scale(0.8); /* More smaller */
        }
        .frameless-slide.side-2::after {
            background: rgba(0, 0, 0, 0.5); /* More overlay */
        }

        /* Side previews - third level (and beyond) */
        .frameless-slide.side-3 img,
        .frameless-slide:not(.active):not(.side-1):not(.side-2) img {
            filter: brightness(0.3); /* Much darker */
            transform: scale(0.7); /* Much smaller */
        }
        .frameless-slide.side-3::after,
        .frameless-slide:not(.active):not(.side-1):not(.side-2)::after {
            background: rgba(0, 0, 0, 0.7); /* Heavy overlay */
        }

        /* Navigation Arrows */
        .frameless-slideshow-nav {
            cursor: pointer;
            position: absolute;
            top: 50%;
            transform: translateY(-50%);
            width: auto;
            padding: 1rem;
            color: white;
            font-weight: bold;
            font-size: 2rem;
            transition: 0.3s ease;
            border-radius: 0.5rem; /* Rounded corners for buttons */
            user-select: none;
            background-color: rgba(0, 0, 0, 0.5);
            z-index: 10; /* Above slides and wrappers */
            display: flex; /* For centering content inside */
            align-items: center;
            justify-content: center;
        }

        .frameless-slideshow-nav.prev {
            left: 1rem;
        }

        .frameless-slideshow-nav.next {
            right: 1rem;
        }

        .frameless-slideshow-nav:hover {
            background-color: rgba(0, 0, 0, 0.8);
            color: #0066CC; /* Highlight on hover */
        }
        /* Ajouté: style pour les flèches désactivées */
        .frameless-slideshow-nav.disabled {
            pointer-events: none;
            opacity: 0.5;
            cursor: not-allowed;
        }

        .frameless-slideshow-dots {
             position: relative;
             margin-top: 1.5rem;
             text-align: center;
              //background-color: rgba(0, 0, 0, 0.4);
             z-index: 1; /* Optionnel : s’assurer que c’est au-dessus du footer */
         }

.frameless-dot {
    position: relative;
    width: 30px;
    height: 30px;
    margin: 0 5px;
    cursor: pointer;
    display: inline-block;
    background: none;
    border: none;
    padding: 0;
}

.frameless-dot::after {
    content: '';
    position: absolute;
    top: 50%;
    left: 50%;
    width: 10px;
    height: 10px;
    background-color: #222;
    border-radius: 50%;
    transform: translate(-50%, -50%);
    transition: background-color 0.3s ease;
    pointer-events: none;
}

/* Survol = point bleu */
.frameless-dot:hover::after {
    background-color: #11AAFF;
       width: 14px;
    height: 14px;
}

/* Actif = point bleu aussi */
.frameless-dot.active::after {
    background-color: #0066CC;
}


    </style>
</head>
<body class="min-h-screen">
    <div id="console-wrapper">
        <div id="console-container">
            <div id="console-output" class="text-left mono text-base text-gray-300">
            </div>
            <div id="console-blur-overlay"></div>
        </div>
    </div>

    <div id="main-content">
        <section class="min-h-screen flex flex-col justify-center items-center px-4 py-20 text-center">
            <h1 class="mono text-4xl md:text-6xl font-bold mb-6">
                <span class="wake-blue title-outline">WAKE</span> <span class="title-outline">JIT ENGINE</span>
            </h1>
            <p class="text-xl md:text-2xl max-w-3xl mx-auto mb-12">
                The Instant Native Engine.<br>
                Run live C99 & shaders, package securely, and deploy instantly.
            </p>
            <div class="flex flex-wrap justify-center gap-4">
                <a href="#" class="bg-wake-blue hover-bg-wake-blue text-white px-6 py-3 rounded-md font-medium transition flex items-center">
                    ⬇ Download Wake
                </a>
                <a href="#" class="border border-wake-blue text-wake-blue px-6 py-3 rounded-md font-medium transition flex items-center">
                    ▶ Live Demo
                </a>
                <a href="https://discord.gg/dKxkvDK37e" class="border border-gray-600 text-gray-300 px-6 py-3 rounded-md font-medium transition flex items-center">
                    💬 Join Discord
                </a>
            </div>
        </section>

        <section class="py-20 px-0 max-w-full overflow-hidden">
            <h2 class="mono text-3xl font-bold mb-12 text-center">Wake in Action</h2>
            <div class="frameless-slideshow-container" id="framelessSlideshowContainer">
                <div class="frameless-slideshow-wrapper" id="framelessSlideshowWrapper">
                    <div class="frameless-slide">
                        <img src="images/frameless/01.png" alt="Wake in Action 1" draggable="false">
                    </div>
                    <div class="frameless-slide">
                        <img src="images/frameless/02.png" alt="Wake in Action 2" draggable="false">
                    </div>
                    <div class="frameless-slide">
                        <img src="images/frameless/03.png" alt="Wake in Action 3" draggable="false">
                    </div>
                    <div class="frameless-slide">
                        <img src="images/frameless/04.gif" alt="Wake in Action 4" draggable="false">
                    </div>
                    <div class="frameless-slide">
                        <img src="images/frameless/05.gif" alt="Wake in Action 5" draggable="false">
                    </div>
                </div>

                <a class="frameless-slideshow-nav prev" id="framelessNavPrev">&#10094;</a>
                <a class="frameless-slideshow-nav next" id="framelessNavNext">&#10095;</a>

               
            </div>
             <div class="frameless-slideshow-dots" id="framelessSlideshowDots"></div>
        </section>

        <section class="py-20 px-4 max-w-6xl mx-auto">
            <h2 class="mono text-3xl font-bold mb-12 text-center">Wake Ecosystem</h2>
            <div class="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
                <div class="module-card p-6 rounded-lg relative overflow-hidden">
                    <div class="absolute inset-0 z-0 opacity-20">
                        <img src="https://via.placeholder.com/400x300?text=Shader+Code" alt="Shader example" class="w-full h-full object-cover">
                    </div>
                    <div class="relative z-10">
                        <h3 class="mono text-xl font-bold mb-2">ShaderPlayground</h3>
                        <p class="text-gray-300 text-sm mb-4">Écrire et tester des shaders GLSL en live</p>
                        <div class="flex justify-between items-center">
                            <span class="text-xs px-2 py-1 bg-green-900/50 text-green-300 rounded backdrop-blur-sm">Free / Pro</span>
                            <button class="text-xs text-wake-blue">Learn more →</button>
                        </div>
                    </div>
                </div>
                <div class="module-card p-6 rounded-lg relative overflow-hidden">
                    <div class="absolute inset-0 z-0 opacity-20">
                        <img src="https://via.placeholder.com/400x300?text=Wallpaper+Abstract" alt="Wallpaper example" class="w-full h-full object-cover">
                    </div>
                    <div class="relative z-10">
                        <h3 class="mono text-xl font-bold mb-2">SpinView</h3>
                        <p class="text-gray-300 text-sm mb-4">Créer des wallpapers dynamiques contrôlés par l'utilisateur</p>
                        <div class="flex justify-between items-center">
                            <span class="text-xs px-2 py-1 bg-green-900/50 text-green-300 rounded backdrop-blur-sm">Free</span>
                            <button class="text-xs text-wake-blue">Learn more →</button>
                        </div>
                    </div>
                </div>
                <div class="module-card p-6 rounded-lg relative overflow-hidden">
                    <div class="absolute inset-0 z-0 opacity-20">
                        <img src="https://via.placeholder.com/400x300?text=Typography+Font" alt="Typography example" class="w-full h-full object-cover">
                    </div>
                    <div class="relative z-10">
                        <h3 class="mono text-xl font-bold mb-2">WakeFont</h3>
                        <p class="text-gray-300 text-sm mb-4">Moteur typographique vectoriel animé</p>
                        <div class="flex justify-between items-center">
                            <span class="text-xs px-2 py-1 bg-yellow-900/50 text-yellow-300 rounded backdrop-blur-sm">Coming soon</span>
                            <button class="text-xs text-wake-blue">Learn more →</button>
                        </div>
                    </div>
                </div>
                <div class="module-card p-6 rounded-lg relative overflow-hidden">
                    <div class="absolute inset-0 z-0 opacity-20">
                        <img src="https://via.placeholder.com/400x300?text=Keyboard+Technology" alt="Keyboard example" class="w-full h-full object-cover">
                    </div>
                    <div class="relative z-10">
                        <h3 class="mono text-xl font-bold mb-2">GlassTyping</h3>
                        <p class="text-gray-300 text-sm mb-4">Moteur de clavier dynamique ultra-rapide</p>
                        <div class="flex justify-between items-center">
                            <span class="text-xs px-2 py-1 bg-purple-900/50 text-purple-300 rounded backdrop-blur-sm">Pro only</span>
                            <button class="text-xs text-wake-blue">Learn more →</button>
                        </div>
                    </div>
                </div>
            </div>
            <div class="mt-8 text-center">
                <span class="mono text-sm text-gray-500">Everything is <span class="text-wake-blue">Powered by Wake</span></span>
            </div>
        </section>

        <section class="py-32 px-4 max-w-3xl mx-auto text-center">
            <div class="space-y-8">
                <p class="manifesto-line mono text-2xl md:text-3xl">You don't wait for your code to compile.</p>
                <p class="manifesto-line mono text-2xl md:text-3xl">You see it evolve.</p>
                <p class="manifesto-line mono text-2xl md:text-3xl">You don't just build interfaces.</p>
                <p class="manifesto-line mono text-2xl md:text-3xl">You grow them.</p>
                <p class="manifesto-line mono text-2xl md:text-3xl wake-blue">Welcome to Wake.</p>
            </div>
        </section>

        <footer class="bg-black py-12 px-4 border-t border-gray-900">
            <div class="max-w-6xl mx-auto">
                <div class="flex flex-col md:flex-row justify-between items-center">
                    <div class="mb-6 md:mb-0">
                        <h3 class="mono text-xl font-bold mb-2">WAKE JIT ENGINE</h3>
                        <p class="text-gray-500 text-sm">The Power of a Live Engine</p>
                    </div>
                    <div class="flex space-x-6">
                        <a href="#" class="text-gray-400 hover:text-wake-blue transition">GitHub</a>
                        <a href="#" class="text-gray-400 hover:text-wake-blue transition">Discord</a>
                        <a href="#" class="text-gray-400 hover:text-wake-blue transition">Licence WPL-1.0</a>
                    </div>
                </div>
                <div class="mt-8 pt-8 border-t border-gray-900 text-center text-gray-500 text-sm">
                    <a href="https://majify.tech" target="_blank" class="hover:underline">© 2025 Majify Technologies</a>
                    <p class="mt-2">Powered by Wake. No trackers. No analytics. Just execution.</p>
                </div>

            </div>
        </footer>
    </div>

    <script>
        document.addEventListener('DOMContentLoaded', () => {
            const consoleOutput = document.getElementById('console-output');
            const mainContent = document.getElementById('main-content');
            const consoleWrapper = document.getElementById('console-wrapper');
            const consoleBlurOverlay = document.getElementById('console-blur-overlay');

            const introMessages = [
                '> Wake Runtime v1.0',
                '> Compiling live interface...',
                '> Ready.'
            ];

            const backgroundMessages = [
                '> Running background tasks...',
                '> <span class="text-yellow-400">Warning, website is under construction</span>...',
                '> Ready for user input'
            ];

            function addConsoleLine(message, targetElement, delay = 0, isYellow = false) {
                return new Promise(resolve => {
                    setTimeout(() => {
                        const newLine = document.createElement('div');
                        newLine.className = 'console-text mt-1';
                        if (isYellow) {
                            newLine.innerHTML = message;
                        } else {
                            newLine.textContent = message;
                        }
                        targetElement.appendChild(newLine);
                        targetElement.scrollTop = targetElement.scrollHeight;
                        resolve();
                    }, delay);
                });
            }

            async function animateIntroConsole() {
                mainContent.style.opacity = '1.0';
                for (let i = 0; i < introMessages.length; i++) {
                    await addConsoleLine(introMessages[i], consoleOutput, 1);
                }
            }

            async function animateBackgroundConsole() {
                await addConsoleLine(backgroundMessages[0], consoleOutput, Math.random() * 500 + 500);
                await addConsoleLine(backgroundMessages[1], consoleOutput, Math.random() * 500 + 500, true);
                await addConsoleLine(backgroundMessages[2], consoleOutput, Math.random() * 500 + 500);
            }

            let ticking = false; // Variable pour limiter les mises à jour

            window.addEventListener('scroll', () => {
                if (!ticking) {
                    window.requestAnimationFrame(() => {
                        const scrollY = window.scrollY;

                        // Parallax effect on the wrapper
                        consoleWrapper.style.transform = `translateY(${scrollY * 0.2}px)`;

                        // Code pour le blur progressif
                        const scrollStart = 0;
                        const scrollEnd = 400; // Augmente le flou sur les 400 premiers pixels
                        const minBlur = 0;
                        const maxBlur = 10; // Flou max à 10px

                        let blurAmount = 0;
                        if (scrollY <= scrollStart) {
                            blurAmount = minBlur;
                        } else if (scrollY >= scrollEnd) {
                            blurAmount = maxBlur;
                        } else {
                            blurAmount = minBlur + ((scrollY - scrollStart) / (scrollEnd - scrollStart)) * (maxBlur - minBlur);
                        }

                        consoleBlurOverlay.style.backdropFilter = `blur(${blurAmount}px)`;
                        ticking = false; // Réinitialise le flag après la mise à jour
                    });
                    ticking = true; // Empêche d'autres mises à jour tant que le cadre n'est pas rendu
                }
            });

            // Animation sequence
            animateIntroConsole().then(() => {
                animateBackgroundConsole();
            });

            // Intersection Observer for scroll animations
            const observer = new IntersectionObserver((entries) => {
                entries.forEach(entry => {
                    if (entry.isIntersecting) {
                        entry.target.classList.add('visible');
                    }
                });
            }, {
                threshold: 0.1
            });

            document.querySelectorAll('.slide-up').forEach(el => observer.observe(el));
            document.querySelectorAll('.manifesto-line').forEach((el, i) => {
                setTimeout(() => observer.observe(el), i * 200);
            });

            // --- Frameless Slideshow Logic (Infinite Loop) ---
         
            let framelessSlideshowTimer;
            const framelessWrapper = document.getElementById('framelessSlideshowWrapper');
            let framelessSlides = Array.from(document.querySelectorAll('.frameless-slide')); // Convert NodeList to Array
            const framelessDotsContainer = document.getElementById('framelessSlideshowDots');
            const framelessNavPrev = document.getElementById('framelessNavPrev');
            const framelessNavNext = document.getElementById('framelessNavNext');
            let TotalSlides = framelessSlides.length;
            let framelessSlideIndex = TotalSlides*10;//prevent negative values

            let isDragging = false;
            let startPos = 0;
            let currentTranslate = 0;
            let prevTranslate = 0;
            let transitionEndPromiseResolver = null; // To resolve the promise when transition ends

            // Generate dots dynamically for original slides only
            for (let i = 0; i < TotalSlides; i++) {
                const dot = document.createElement("span");
                dot.classList.add("frameless-dot");
                // Attacher l'événement click aux points pour les sauts multiples
                dot.addEventListener('click', () => framelessCurrentSlide(i)); 
                framelessDotsContainer.appendChild(dot);
            }

            // Empêcher le comportement de glisser-déposer par défaut des images
            framelessWrapper.addEventListener('dragstart', (e) => {
                e.preventDefault();
            });

            const setSlideClasses = () => {
            
            
               updateSlidePositions();
               
               
                let frameIndex = ((framelessSlideIndex) + TotalSlides) % TotalSlides;
                        
                framelessSlides.forEach((slide, index) => {
                    slide.classList.remove('active', 'side-1', 'side-2', 'side-3');
                                     
                    const diff = index - frameIndex;
                    const absDiff = Math.abs(diff);
     
                    if (absDiff === 0) {
                        slide.classList.add('active');
                    } else if (absDiff === 1 || absDiff==TotalSlides-1) {
                        slide.classList.add('side-1');
                    } else if (absDiff === 2 || absDiff==TotalSlides-2) {
                        slide.classList.add('side-2');
                    } else if (absDiff >= 3 || absDiff==TotalSlides-3) {
                        slide.classList.add('side-3');
                    }
                });

                let currentOriginalIndex = (frameIndex );

                document.querySelectorAll('.frameless-dot').forEach((dot, index) => {
                    dot.classList.toggle('active', index === currentOriginalIndex);
                });
            };

            // This function applies the transform, and handles adding/removing no-transition
            // It now also dynamically sets the transition duration
            const applyTransform = (smooth = true, duration = 600) => {
            
                currentTranslate = -framelessSlideIndex * framelessSlides[0].offsetWidth;
                
                // Set transition duration dynamically
                framelessWrapper.style.transitionDuration = `${duration}ms`;

                if (smooth) {
                    framelessWrapper.classList.remove('no-transition');
                } else {
                    framelessWrapper.classList.add('no-transition');
                }
                framelessWrapper.style.transform = `translateX(${currentTranslate}px)`;
            };
            


            // Main function to move the slides
            const framelessShowSlides = async (targetIndex, smooth = true) => {
                clearTimeout(framelessSlideshowTimer); // Stop auto-advance for now

                const oldIndex = framelessSlideIndex;
                const slidesMoved = Math.abs(targetIndex - oldIndex);
                
                // Calculate dynamic duration: faster for bigger jumps
                // Base duration for 1 slide: 600ms. Max speed up to 300ms for many slides.
                const dynamicDuration = Math.max(300, 600 - (slidesMoved * 50)); // Example calculation
                // Cap duration to a reasonable max/min
                const actualTransitionDuration =  Math.min(600, Math.max(300, dynamicDuration)) ;
                
                framelessSlideIndex = targetIndex; // Update current index immediately
                setSlideClasses(); // Update classes instantly for visual feedback

                // Apply transform (with or without transition and dynamic duration)
                applyTransform(true, actualTransitionDuration); 

                // Restart auto-advance timer
                framelessSlideshowTimer = setTimeout(() => {
                    framelessPlusSlides(1); // Auto-advance
                }, 5000); // 5 seconds
                
                
            };
           

/*           
         function updateSlidePositions() {
             const slideWidth = framelessSlides[0].offsetWidth;
             const visibleCount = 2;
             const total = framelessSlides.length;
             
             let frameIndex = ((framelessSlideIndex) + TotalSlides) % TotalSlides;
             let framepos = Math.floor(framelessSlideIndex/TotalSlides)*TotalSlides;

             for (let i = 0; i < total; i++) {
                  let offset = framepos * slideWidth + slideWidth/2;
                  if(i<visibleCount && frameIndex>total-visibleCount){
                     offset+=slideWidth*TotalSlides;
                  }
                  
                  if(i>total-visibleCount && frameIndex<visibleCount){
                     offset-=slideWidth*TotalSlides;
                  }
                  
                  framelessSlides[i].style.transform = `translateX(${offset}px)`;
                  
             }
         }
*/
function updateSlidePositions() {
    const total = framelessSlides.length;
  //  const containerWidth = framelessWrapper.parentElement.offsetWidth;

    // Calcul de l'index réel dans la plage [0, total)
    const frameIndex = framelessSlideIndex % total;
    const loopIndex = Math.floor(framelessSlideIndex / total);

    // Calcule les positions X cumulées pour chaque slide
    let slideOffsets = [];
    let cumulativeOffset = 0;

    for (let i = 0; i < total; i++) {
        slideOffsets.push(cumulativeOffset);
       // cumulativeOffset += framelessSlides[i].offsetWidth;
        cumulativeOffset += framelessSlides[i].getBoundingClientRect().width;
        //cumulativeOffset += 500;
        // const img = framelessSlides[i].querySelector('img');
        // if (img) {
        //     cumulativeOffset += img.naturalWidth;
        // }

    }
    console.log("Total cumulativeOffset =", cumulativeOffset);

    // Largeur totale réelle des slides
    const totalslideWidth = cumulativeOffset;

    // Position absolue de la slide active dans l'ensemble étendu
   // const activeAbsoluteOffset = loopIndex * totalslideWidth + slideOffsets[frameIndex];

    // Décalage à appliquer pour centrer la slide active dans le viewport
   // const centerShift = activeAbsoluteOffset - ((containerWidth - framelessSlides[frameIndex].offsetWidth) / 2);

    // Applique le translateX à chaque slide
    for (let i = 0; i < total; i++) {
        const absoluteSlideOffset = loopIndex * totalslideWidth + slideOffsets[i];
        //let offset = absoluteSlideOffset - centerShift;
        let offset = absoluteSlideOffset ;

         // Gestion du wrap circulaire pour affichage fluide
         const visibleCount = 2;
         if (i < visibleCount && frameIndex > total - visibleCount) {
               offset += totalslideWidth;
         } else if (i > total - visibleCount && frameIndex < visibleCount) {
               offset -= totalslideWidth;
         }
         
        framelessSlides[i].style.transform = `translateX(${offset}px)`;
    }
}




            // Event listeners for nav buttons 
            framelessNavPrev.addEventListener('click', () => framelessPlusSlides(-1));
            framelessNavNext.addEventListener('click', () => framelessPlusSlides(1));
            
            window.framelessPlusSlides = (n) => {
                framelessShowSlides(framelessSlideIndex + n, true);
            };

            window.framelessCurrentSlide = (dotIndex) => {
                // When clicking a dot, go directly to the corresponding real slide (+1 because of prepended clone)
                 let framepos = Math.floor(framelessSlideIndex/TotalSlides)*TotalSlides;
                framelessShowSlides(framepos + dotIndex , true);
            };

            // --- Dragging Logic ---
            const framelessSlideshowContainer = document.getElementById('framelessSlideshowContainer');

            framelessSlideshowContainer.addEventListener('mousedown', (e) => {
                isDragging = true;
                startPos = e.clientX;
                framelessSlideshowContainer.classList.add('grabbing');
                // Apply no-transition immediately for drag smoothness
                framelessWrapper.classList.add('no-transition'); 
                prevTranslate = currentTranslate;
                clearTimeout(framelessSlideshowTimer); 
                // Abort any ongoing CSS transition immediately for drag
                framelessWrapper.style.transform = `translateX(${currentTranslate}px)`;
            });

            framelessSlideshowContainer.addEventListener('mousemove', (e) => {
                if (!isDragging) return;
                e.preventDefault(); // Prevent default browser drag behavior
                const currentPosition = e.clientX;
                const diff = currentPosition - startPos;
                currentTranslate = prevTranslate + diff;
                framelessWrapper.style.transform = `translateX(${currentTranslate}px)`;
            });

            const endDrag = () => {
                if (!isDragging) return; // Only run if actually dragging
                isDragging = false;
                framelessSlideshowContainer.classList.remove('grabbing');
                
                // Calculate target index based on drag distance
                const slideWidth = framelessSlides[0].offsetWidth;
                const movedBy = currentTranslate - prevTranslate;

                let targetIndex = framelessSlideIndex; 

                // Determine the direction of the drag and adjust targetIndex
                if (movedBy < -slideWidth / 4) { // Swiped left enough (go next)
                    targetIndex++;
                } else if (movedBy > slideWidth / 4) { // Swiped right enough (go prev)
                    targetIndex--;
                }
                
              //  // Ensure target index stays within the "cloned" bounds for the snap calculation
              //  // The framelessShowSlides function will then handle the jump to real index
              //  if (targetIndex < 0) targetIndex = 0;
              //  if (targetIndex >= TotalSlides) targetIndex = TotalSlides - 1;


                // Call framelessShowSlides to snap to the target index with a smooth transition
                framelessShowSlides(targetIndex, true); 
            };

            framelessSlideshowContainer.addEventListener('mouseup', endDrag);
            framelessSlideshowContainer.addEventListener('mouseleave', endDrag); // Crucial for when mouse leaves container

            // Initial setup - important to start at the first "real" slide (index 1)
            applyTransform(false); // Start without transition to position correctly
            setSlideClasses(); // Apply initial classes

            // Recalculate position on window resize to ensure correct centering
            window.addEventListener('resize', () => {
                // Ensure no transition during resize to prevent visual glitches
                framelessWrapper.classList.add('no-transition');
                applyTransform(false); 
                setSlideClasses(); 
                // Force a reflow to apply no-transition immediately
                framelessWrapper.offsetWidth; 
                // Re-enable transition after a very short delay
                setTimeout(() => framelessWrapper.classList.remove('no-transition'), 50);
            });

            // Start auto-advance after initial setup
            framelessSlideshowTimer = setTimeout(() => {
                framelessPlusSlides(1);
            }, 5000);
        });
    </script>
</body>
</html>